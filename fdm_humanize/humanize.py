...
def analyse_file(filename) -> tuple[list, list, int]:
    ...
    print(os.path.join("static/extracted", filename[:-7]))
    if filename.endswith(".tar.gz") and not os.path.exists(os.path.join("static/extracted", filename[:-7])):
        logger.info("Extracting %s", filename)
        extract_logs(os.path.join("static/uploaded_files", filename), os.path.join("static/extracted", filename[:-7]))
    # open all relevant files in the extracted directory
    if os.path.exists(os.path.join("static/extracted", filename[:-7])):
        logger.info("Analysing %s", filename)
        for root, dirs, files in os.walk(os.path.join("static/extracted", filename[:-7])):
